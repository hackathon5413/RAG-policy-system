...
async def run_hackrx(
    ...
    try:
        ...
        result = await process_document_and_answer(
            str(request.documents), request.questions
        )
        ...
        if result["success"]:
            ...
            logger.info(
                f"Response - Success: True, Answers count: {len(response.answers)}"
            )
            return response
        else:
            ...
            response = HackRXResponse(answers=result["answers"])
            logger.info(
                f"Response - Success: False, Error answers count: {len(response.answers)}"
            )
            return response
...
async def test_local_file(
    request: LocalTestRequest, token: str = Depends(verify_token)
) -> HackRXResponse:
    ...
    try:
        ...
        from .document_processor import answer_questions, process_local_document
        ...
        if not result["success"]:
            return HackRXResponse(
                answers=[f"Error: {result['error']}" for _ in request.questions]
            )
        ...
        logger.error(f"Error processing local file: {e!s}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error processing local file: {e!s}",
        )
