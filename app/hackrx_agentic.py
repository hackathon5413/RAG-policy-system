...
async def process_hackrx_agentic(
    ...
    try:
        ...
        if apis_needed:
            ...
            for api_url, result in zip(apis_needed, results, strict=False):
                api_results[api_url] = (
                    result
...
async def classify_apis_needed(pdf_content: str, questions: list[str]) -> list[str]:
    try:
        ...
        prompt = template.render(pdf_content=pdf_content, questions=questions)
...
async def generate_final_answers(
    pdf_content: str, api_results: dict[str, Any], questions: list[str]
) -> list[str]:
    try:
        template = jinja_env.get_template("agentic_answerer.j2")
        prompt = template.render(
            pdf_content=pdf_content, api_results=api_results, questions=questions
        )
        ...
        return [f"Error: {e!s}" for _ in questions]
